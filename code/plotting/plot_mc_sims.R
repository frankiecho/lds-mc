...
alpha_vec <- c(0, 15, 30)
alpha_lab <- c(`0` = 'θ=0', `20` = 'θ=15', `40` = 'θ=30')
...
ce_df <- lapply(runs, \(i) read_csv(paste0("output/ce_df_param_search_", i, "_", nsims, ".csv"), show_col_types = F)) %>%
  ...
  lapply(\(x) dplyr::select(x, 'cvar_mstd', 'var', 'alpha')) %>%
  lapply(\(x) pivot_wider(x, names_from = 'var', values_from = 'cvar_mstd'))
...
fcn_generate_mc_lineplots <- function(variable, axis_title = NULL, hide_y = TRUE) {
  ...
  p <- ce_df_comb %>%
    ...
    ggplot(aes(x = !!as.symbol(variable), color = sig_diff)) +
    geom_hline(yintercept = 0) +
    geom_errorbar(aes(ymin = lb, ymax = ub), width = 0, linewidth = 0.5) +
    geom_point(aes(y = median)) +
    scale_y_continuous("Change from M-SD", limits = c(-0.08, 0.08), labels = scales::percent) +
...
ggsave('plots/mc_sim_plot.png', plt, scale = 2, width = 2000, height = 1000, units = 'px')
