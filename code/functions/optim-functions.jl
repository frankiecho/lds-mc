...
function fcn_optim_cvar(Y; p=ones(size(Y, 2), 1) / size(Y, 2), budget=1, β::Real=0.9, λ::Float64=1.0)
...
function fcn_optim_mstd(Y; budget=1, λ=1, p=ones(size(Y, 2), 1) / size(Y, 2), β::Real=0)
    if (λ == 0)
        x = fcn_optim_ev(Y; p=p, budget=budget)
        return (x)
    end
